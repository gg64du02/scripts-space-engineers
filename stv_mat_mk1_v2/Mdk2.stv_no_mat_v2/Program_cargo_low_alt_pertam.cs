    ...
    partial class Program : MyGridProgram
    {
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            else
            {
                ...
                double AngleRollMaxAcc = Math.Atan((TWR - 1) / 1);
                Echo("MaxAngleMax(deg):" + Math.Round(AngleRollMaxAcc * 180 / Math.PI,1));
                Echo("MaxAngleMax(rad:" + Math.Round(AngleRollMaxAcc, 1));
                Echo("thres:" + Math.Round(Math.Cos(AngleRollMaxAcc), 1));
                float varHorThres = (float) (Math.Cos(AngleRollMaxAcc) * 100);
                Echo("varHorThres:" + Math.Round(varHorThres, 1));

                //error_sideways_speed = MyMath.Clamp((float)error_sideways_speed.Length(), 0f, 11f)
                //error_sideways_speed = MyMath.Clamp((float)error_sideways_speed.Length(), 0f, 90f)
                error_sideways_speed = MyMath.Clamp((float)error_sideways_speed.Length(), 0f, varHorThres)
                ...
                if (dts > 0)
                {
                    if (vec3Dtarget != new Vector3D(0, 0, 0))
                    {
                        ...
                        //Debug.PrintChat("FR:"+ falling_range);
