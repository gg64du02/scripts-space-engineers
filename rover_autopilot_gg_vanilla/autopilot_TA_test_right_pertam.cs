...
public void Main(string argument, UpdateType updateSource)
{
	...
	// is 0 because it does not cost anythojng to move from starting node
	gscore.Add(node,0);
		...
		if(ourDestinationNode == node){
			...
			foreach(Node neighbor in neighbors){
				...
				if(gscore.ContainsKey(neighbor)==false){
					gscore.Add(neighbor,Math.Sqrt(distanceSquarred(node.position,neighbor.position)));
				}
				else{
					Echo("gscore["+neighbor.index+"]:"+gscore[neighbor]);
				}
			}
	...
	*/
	...
	while(true){
		...
		Echo("closelist.Count:"+closelist.Count);
		
		if(ourDestinationNode == node){
			break;
		}
		
        //py close_set.add(current)
		closelist.Add(node);
		...
		foreach(Node neighbor in neighbors){
            ...
            // py tentative_g_score = gscore[current] + heuristic(current, neighbor)
			double tentative_g_score = gscore[node] + heuristic(node.position,neighbor.position);
			Echo("tentative_g_score:"+tentative_g_score);
			
            // py if neighbor in close_set 
			// py and tentative_g_score >= gscore.get(neighbor, 0):
            // py    continue
			Echo("nei2");
			...
			if(gscore.ContainsKey(neighbor)==false){
				gscore.Add(neighbor,Math.Sqrt(distanceSquarred(neighbor.position,node.position)));
				Echo("gscore add for:"+neighbor.index);
			}
			if(fscore.ContainsKey(neighbor)==false){
				fscore.Add(neighbor,gscore[neighbor]+Math.Sqrt(distanceSquarred(neighbor.position,ourDestinationNode.position)));
				Echo("fscore add for:"+neighbor.index);
			}
			...
			if(listHeapNodes.Contains(neighbor)==false){
				testEnd = true;
			}
	...
	while(true){
	   ...
	   if(openlist.Count != 1){
			foreach(Node testFnode in openlist){
				//bugfix
				if(closelist.Contains(testFnode) == true){
					continue;
				}
					
				Echo("testFnode.index:"+testFnode.index);
				if(openlist.Count==1){
					Echo("openlist[0]:"+openlist[0]);
				}
				//bugfix ?
				if(fscore.ContainsKey(testFnode)==false){
					fscore.Add(testFnode, gscore[testFnode]+Math.Sqrt(distanceSquarred(node.position,testFnode.position)));
					Echo("fscore added for:"+testFnode.index);
				}
				
				// Echo("fscore[testFnode]:"+fscore[testFnode]);
				Echo("fscore["+testFnode.index+"]:"+fscore[testFnode]);
				if(tmpF==-1){
		...
		//7 if not:
			...
			foreach(Node neighbor in neighbors){
				...
				if(gscore.ContainsKey(neighbor)==false){
					gscore.Add(neighbor, Math.Sqrt(distanceSquarred(node.position,neighbor.position)));
					Echo("gscore added for:"+neighbor.index);
				}
