...
for j in range(2048):
    for k in range(2048):
        ...
        if ((converted_to_bool_underground_array[j, k] == 1)):
            ...
            # trying to remove the same pattern amongst the remaining picture
            for p in range(2048):
                for m in range(2048):
                    # print("lol2")
                    one_not_matching = False
                    # # check if the detected pattern is the same as the
                    if ((converted_to_bool_underground_array[p, m] == 1)):
                        print("len pixelWithValues",len(pixelWithValues))
                        for currentPatternPixel in range(len(pixelWithValues)):
                            # print("currentPatternPixel",currentPatternPixel)
                            print("pixelWithValues[currentPatternPixel]",pixelWithValues[currentPatternPixel])
                            print(pixelWithValues[currentPatternPixel])
                            print(pixelWithValues[currentPatternPixel[0]])
                            if(data_ore_layer[pixelWithValues[currentPatternPixel][0][0]+p,pixelWithValues[currentPatternPixel][0][1]+m]==pixelWithValues[currentPatternPixel][1][0]):
                                print("lol3")
                                pass
                            else:
                                one_not_matching = True

                    if(one_not_matching == False):
